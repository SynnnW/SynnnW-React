...
// ─────────────────────────────────────────────
function VideoPlayer({ url, label }) {
  ...
  return (
    <div className="db-video-player-wrap">
      <div className="db-video-aspect">
        {isPlaying ? (
          <iframe
            ref={iframeRef}
            src={previewUrl}
          ...
          <div style={{ width: '100%', height: '100%', background: '#0a0a0f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <i className="fa-solid fa-film" style={{ fontSize: '2rem', color: 'rgba(167,139,250,0.3)' }} />
          </div>
        )}
        <div className={`db-video-overlay ${isPlaying ? 'playing' : ''}`}>
          {!isPlaying && (
            <button className="db-video-play-btn" onClick={handlePlay}>
              <i className="fa-solid fa-play" />
            </button>
          )}
          {isPlaying && (
            <div className="db-video-controls" onClick={(e) => e.stopPropagation()}>
              <button className="db-video-ctrl-btn" onClick={handleStop}>
      ...
      <div className="db-video-label">{label}</div>
    </div>
  );
}
        ...
        {main.length > 0 && (
          <>
            ...
            <div className="db-video-grid" style={{ marginBottom: '24px' }}>
              {main.map((v, i) => <VideoPlayer key={i} url={v.url} label={v.label} />)}
            </div>
          </>
        )}
        {rev.length > 0 && (
          <>
            ...
            <div className="db-video-grid">
              {rev.map((v, i) => <VideoPlayer key={i} url={v.url} label={v.label} />)}
            </div>
          </>
        )}
      </>
      ...
      <main className="db-main">
        ...
        <div className="db-main-header">
          <div className="db-main-header-left">
            ...
            <p className="db-main-header-sub">Selamat datang di dashboard klien SynnnW Studio</p>
          </div>
          {/* Notification Bell */}
