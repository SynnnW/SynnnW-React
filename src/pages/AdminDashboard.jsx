...
export default function AdminDashboard() {
  ...
  const NAV_ITEMS = [
    ...
    { id: 'clientPrices', icon: 'fa-coins', label: 'Harga Klien' },
    ...
    <>
      ...
      <div className="da-app">
        ...
        <div className="da-main">
          ...
          {activeTab === 'clientPrices' && <ClientPricesTab orders={orders} />}
...
function DashboardEditorTab({ orders }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'dashboardContent', targetUserId), (snap) => {
      if (snap.exists()) {
        const d = snap.data();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const targetUserId = selectedOrder?.userId;
      if (!targetUserId) throw new Error('userId tidak ditemukan');
      await setDoc(doc(db, 'dashboardContent', targetUserId), {
