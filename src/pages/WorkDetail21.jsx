...
const DATA = {
  ...
  titleSub:   'Educational Video Documentation',
  year:       '2026',
  production: 'Educational Content',
  pembina:    null,
  editor:     'Aldo Leo Saputra',
  youtubeId:  null,
  badges:     ['Video Editing', 'Educational'],
  awards:     [],

  descEn: 'Professional biology practicum video editing with broadcast standards. Clear focus on every moment, consistent color correction, and optimal lighting for learning documentation. Educational narrative flow that guides students from setup through execution to results.',
  descId: 'Editing video praktikum biologi dengan standar broadcast. Sharp focus pada setiap moment, color correction konsisten, dan lighting optimal untuk dokumentasi pembelajaran. Alur naratif pendidikan yang membimbing siswa dari persiapan hingga hasil.',
...
.wd-hero-bg {
  ...
  background:
    radial-gradient(ellipse 55% 55% at 85% 15%, rgba(16,185,129,0.14) 0%, transparent 60%),
...
.wd-badge-video { background: rgba(16,185,129,0.12); border: 1px solid rgba(16,185,129,0.3); color: #10b981; }
.wd-badge-edu { background: rgba(6,182,212,0.12); border: 1px solid rgba(6,182,212,0.3); color: #06b6d4; }
.wd-hero-title {
    ...
    <div className="wd-page" ref={pageRef}>
      <section className="wd-hero">
        ...
        <div className="wd-breadcrumb wd-reveal">
          Work <span>/ Educational Video</span>
        </div>

        <div className="wd-badges wd-reveal wd-d1">
          {d.badges.map((b, i) => (
            <span key={i} className={`wd-badge wd-badge-${b === 'Video Editing' ? 'video' : 'edu'}`}>
              {b}
            </span>
          ))}
        </div>
      ...
      <section className="wd-section">
        <span className="wd-sec-label wd-reveal">02 / Crew</span>
        <h2 className="wd-sec-title wd-reveal wd-d1">Production <em>Team.</em></h2>
