...
.pl-currency-label { font-size: 0.68rem; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; color: var(--text-dim); font-family: 'Outfit', sans-serif; }
.pl-currency-pills { display: flex; gap: 6px; }
.pl-currency-btn {
  padding: 5px 14px; border-radius: 99px;
  font-family: 'Outfit', sans-serif; font-size: 0.66rem; font-weight: 700; letter-spacing: 0.1em;
  border: 1px solid var(--gborder); background: transparent; color: var(--text-dim);
  cursor: pointer; transition: all 0.2s;
}
.pl-currency-btn:hover { color: var(--text); border-color: var(--gborder2); }
.pl-currency-btn.pl-curr-active { background: var(--text); color: var(--bg); border-color: var(--text); }
.pl-currency-usd-note { font-size: 0.62rem; color: var(--text-dim); opacity: 0.6; font-style: italic; font-family: 'Outfit', sans-serif; }
...
.pl-notice {
  ...
  font-family: 'Outfit', sans-serif; font-size: 0.74rem; color: var(--text-dim); line-height: 1.5;
}
.pl-notice i { color: var(--accent3); font-size: 0.75rem; flex-shrink: 0; }
.pl-notice a { color: var(--accent3); text-decoration: underline; text-underline-offset: 3px; transition: opacity 0.18s; }
.pl-notice a:hover { opacity: 0.75; }

/* ── Addon slots ── */
.pl-addon-row { display: flex; flex-direction: column; gap: 8px; margin-top: 4px; padding-top: 18px; border-top: 1px dashed var(--border); }
.pl-addon-label { font-size: 0.6rem; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: var(--text-dim); opacity: 0.6; font-family: 'Outfit', sans-serif; margin-bottom: 2px; }
.pl-addon-card { display: flex; align-items: center; gap: 12px; padding: 10px 14px; background: var(--glass); border: 1px solid var(--gborder); border-radius: 12px; transition: border-color 0.2s; }
.pl-addon-card:hover { border-color: var(--gborder2); }
.pl-addon-icon { width: 32px; height: 32px; border-radius: 10px; background: rgba(139,92,246,0.1); border: 1px solid rgba(139,92,246,0.2); display: flex; align-items: center; justify-content: center; font-size: 0.85rem; color: var(--accent3); flex-shrink: 0; }
.pl-addon-info { flex: 1; }
.pl-addon-name { font-size: 0.82rem; font-weight: 600; color: var(--text); margin: 0 0 2px; }
.pl-addon-short { font-size: 0.66rem; color: var(--text-dim); margin: 0; }
...
/* Format price based on selected currency (IDR or USD) */
const USD_RATE = 15000;
const formatPrice = (n, curr = 'IDR') => {
  if (!n) return null;
  if (curr === 'USD') {
    const usd = (n * 2) / USD_RATE;
...
export default function PriceList({ t = {} }) {
  ...
  // ── Prompt 13: Currency toggle & addon slots ──
  const [currency, setCurrency]     = useState('IDR');
  const [addonSlots, setAddonSlots] = useState({});
  ...
  /* ── Firestore: load addon slots (3 invisible slots per catalog) ── */
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'price_addons'), (snapshot) => {
      const slots = {};
      snapshot.forEach((docSnap) => {
        const data = docSnap.data();
        // Only render if slot has at least a name and price
        if (data.catalog_id && data.name && data.price) {
          if (!slots[data.catalog_id]) slots[data.catalog_id] = [];
          // Max 3 addons per catalog
          if (slots[data.catalog_id].length < 3) {
            slots[data.catalog_id].push({
              id:    docSnap.id,
              name:  data.name,
              short: data.short  || '',
              icon:  data.icon   || 'fa-plus',
              price: data.price,
              unit:  data.unit   || '',
            });
          }
      ...
      <div className="pl-page">
        ...
        {cartCount > 0 && (
          <button className="pl-cart-fab" onClick={() => setCartOpen(true)}>
            ...
            <span className="pl-cart-fab-label">{t.plCartFabLabel} · {formatPrice(cartTotal, currency)}</span>
          </button>
        )}
        ...
        {/* ══════ CURRENCY TOGGLE BAR ══════ */}
        <div className="pl-currency-bar">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span className="pl-currency-label">Currency</span>
            <div className="pl-currency-pills">
              <button
                className={`pl-currency-btn ${currency === 'IDR' ? 'pl-curr-active' : ''}`}
                onClick={() => setCurrency('IDR')}
              >
                IDR
              </button>
              <button
                className={`pl-currency-btn ${currency === 'USD' ? 'pl-curr-active' : ''}`}
          ...
          {currency === 'USD' && (
            <span className="pl-currency-usd-note">
              International prices are 2× IDR rate · Estimates only, rates may change
            </span>
          )}
        </div>
        ...
        <div className="pl-notice">
          ...
          <span>
            ...
            <a href="/terms#how-to" onClick={(e) => { e.preventDefault(); navigate('/terms#how-to'); }}>
              read how this works first
            </a>
            {' '}— termasuk alur order, revisi, dan pembayaran.
          </span>
        </div>
        ...
        <div className="pl-body">
          {CATEGORIES.map((cat) => (
            <section key={cat.id} className="pl-section">
                      ...
                      <div className="pl-card-body" onClick={() => openSheet(item)}>
                        ...
                        <div className="pl-card-info">
                          ...
                          <div className="pl-card-price-row">
                            {item.price > 0 ? (
                              <>
                                <span className="pl-card-price">{formatPrice(item.price, currency)}</span>
                                {item.unit && <span className="pl-card-unit">{item.unit}</span>}
                                {item.origPrice > 0 && (
                                  <span className="pl-card-orig">{formatPrice(item.origPrice, currency)}</span>
                                )}
                              </>
                            ) : item.custom ? (
              ...
              {/* ── Addon slots (3 invisible slots per catalog, filled from Firestore) ── */}
              {addonSlots[cat.id] && addonSlots[cat.id].length > 0 && (
                <div className="pl-addon-row">
                  <p className="pl-addon-label">Add-ons available</p>
                  {addonSlots[cat.id].map((addon) => (
                    <div key={addon.id} className="pl-addon-card">
                      <div className="pl-addon-icon">
                        <i className={`fa-solid ${addon.icon}`} />
                      </div>
                      <div className="pl-addon-info">
                        <p className="pl-addon-name">{addon.name}</p>
                        {addon.short && <p className="pl-addon-short">{addon.short}</p>}
                      </div>
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        <div className="pl-addon-price">{formatPrice(addon.price, currency)}</div>
                        {addon.unit && <div className="pl-addon-unit">{addon.unit}</div>}
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="pl-sheet-header">
                  ...
                  <div className="pl-sheet-hinfo">
                    ...
                    <div className="pl-sheet-price-row">
                      {selected.price > 0 ? (
                        <>
                          <span className="pl-sheet-price">{formatPrice(selected.price, currency)}</span>
                          {selected.unit && <span className="pl-sheet-unit">{selected.unit}</span>}
                          {selected.origPrice > 0 && (
                            <span className="pl-sheet-orig">{formatPrice(selected.origPrice, currency)}</span>
                          )}
                        </>
                      ) : selected.custom ? (
            ...
            <div className="pl-sheet pl-cart-sheet" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="pl-sheet-scroll">
                  ...
                  return (
                    <div key={id} className="pl-cart-item">
                      ...
                      <div className="pl-cart-item-info">
                        ...
                        <span className="pl-cart-item-price">{formatPrice(item.price, currency)} {item.unit}</span>
                      </div>
                ...
                <div className="pl-cart-total">
                  ...
                  <span className="pl-cart-total-num">{formatPrice(cartTotal, currency)}</span>
                </div>
                <div className="pl-sheet-syarat">
                  <i className="fa-solid fa-circle-info" />
